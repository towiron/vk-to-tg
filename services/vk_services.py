...
from .send_post import send_photos, send_file, send_text, send_location, send_poll, send_link
...
def load_collection(collection: Collection):
    ...
    if count > 0:
        for user in collection.find({'forwarding': True}):
            ...
            start_thread(user_id, vk_group_id, tg_channel_id)
    else:
        print('База пуста')
            ...
            if update['type'] == 'wall_post_new':
                ...
                if attachments:
                    ...
                    if len(media_photo) > 0 and len(media_file) > 0:
                        text += f"📌<a href='{post_link}'>ВК пост</a>"
                        await send_photos(chat_id=chat_id, text=text, media_photo=media_photo, downloaded_files=downloaded_photo)
                        ...
                        if len(media_photo) > 0:
                            await send_photos(chat_id=chat_id, text=text, media_photo=media_photo, downloaded_files=downloaded_photo)
                        if len(media_file) > 0:
                            await send_file(chat_id=chat_id, text=text, media_file=media_file, downloaded_files=downloaded_file)
                else:
                    text += f"📌<a href='{post_link}'>ВК пост</a>"
